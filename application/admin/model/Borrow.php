...
class Borrow extends Model
{
	...
	//protected $table = 'borrow';
 	...
 	public static function getList($where,$offset=''){
        ...
        $list = self::field('')->order('id desc')->where($where)->paginate($rows)?:[];
	...
	public static function editsaveData($data){
		Db::startTrans();
		try{
			Db::name('borrow')->where('id', $data['id'])->update($data);
			// 提交事务
			Db::commit();
			return true;    
		} catch (\Exception $e) {
